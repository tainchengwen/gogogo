...
window.LArea = (function() {
    ...
    MobileArea.prototype = {
        ...
        getId:function(province,city,area){
        	 var _self = this;
        	 var resultList=[];
        	 var areaData=_self.data;
        	 var provinceCheck=true;
        	 var cityCheck=true;
        	 var areaCheck=true;
        	 var city0,area0;
                for(var i=0;i<areaData.length;i++){
                    if(province==areaData[i].name){
                    	provinceCheck=false;
                    	resultList.push(areaData[i].id)
                        var cityData=areaData[i].child;
                        for(var j=0;j<cityData.length;j++){
                        	city0=cityData[0].id;
                             if(city==cityData[j].name){
                             	cityCheck=false;
                                resultList.push(cityData[j].id)
                                 if('child' in cityData[j]){
                                        var countyData=cityData[j].child;
                                        for(var k=0;k<countyData.length;k++){
                                        	area0=countyData[0].id;
                                             if(area==countyData[k].name){
                                             	areaCheck=false;
                                                resultList.push(countyData[k].id)
                                             }
                                        }
                                 }else{
                                 	  areaCheck=false;
        ...
        bindEvent: function() {
            ...
            function areaCtrlInit() {
                var areaData=_self.data;
                for(var i=0;i<areaData.length;i++){
                    if(_self.value[0]==areaData[i].id){
                        _self.gearArea.querySelector(".area_province").setAttribute("val", i);
                        var cityData=areaData[i].child;
                        for(var j=0;j<cityData.length;j++){
                             if(_self.value[1]==cityData[j].id){
                                 _self.gearArea.querySelector(".area_city").setAttribute("val",j);
                                 if('child' in cityData[j]){
                                         var countyData=cityData[j].child;
                                        for(var k=0;k<countyData.length;k++){
                                             if(_self.value[2]==countyData[k].id){
                                                 _self.gearArea.querySelector(".area_county").setAttribute("val", k);
                                             }
                                        }
                                 }else{
