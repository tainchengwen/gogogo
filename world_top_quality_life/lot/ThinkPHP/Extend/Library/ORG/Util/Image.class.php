...
class Image {
    ...
    static public function water($source, $water, $savename=null, $alpha=80) {
        ...
        //生成混合图像
        imagecopymerge($sImage, $wImage, 0, 0, 0, 0, $wInfo['width'], $wInfo['height'], $alpha);
	 ...
	 * @param string $font_size			字号
	 * @param string $letter_spacing	字间距
	 * @param string $padding_left		左边距
	 * @param string $padding_top		上边距
     * @return string
     */
    static function buildImageVerify($length=4, $mode=1, $type='png', $width=48, $height=22, $verifyName='verify', $font_size=14, $letter_spacing=10, $padding_left=0, $padding_top=0)
	{
        ...
        imagefilledrectangle($im, 0, 0, $width - 1, $height - 1, $backColor);
        imagerectangle($im, 0, 0, $width - 1, $height - 1, $borderColor);
        $stringColor = imagecolorallocate($im, mt_rand(0, 200), mt_rand(0, 120), mt_rand(0, 120));			// 原有的文字颜色
		$disturbColor_line = imagecolorallocate($im, mt_rand(0, 200), mt_rand(0, 120), mt_rand(0, 120));	// 干扰线颜色，与文字颜色一并随机生成，但可很大几率的保证颜色不同，方便识别
		$disturbColor_circle = imagecolorallocate($im, mt_rand(0, 200), mt_rand(0, 120), mt_rand(0, 120));	// 干扰点颜色，与文字颜色一并随机生成，但可很大几率的保证颜色不同，方便识别
        // 干扰
        for ($i = 0; $i < 10; $i++) {
        ...
        for ($i = 0; $i < $length; $i++) {
            ...
            imagettftext($im, $font_size, 10, $i*$letter_spacing+5+$padding_left, mt_rand(20,30)+$padding_top, $stringColor, $fontPath, $randval{$i});
