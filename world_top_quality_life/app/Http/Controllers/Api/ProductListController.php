...
class ProductListController extends Controller
{
    ...
    public function addProduct(Request $request){
        $validator = Validator::make($request->all(), [
            ...
            'product_name' => 'required',
            'weight' => 'required|numeric',
            'brand_id' => 'required|numeric', //品牌
            'model' => 'required', //型号
            'number' => 'required|numeric', //包装数量
            'product_long' => 'required|numeric',
            'series_id' => 'required|numeric', //系列
            'product_wide' => 'required|numeric',
            'product_height' => 'required|numeric',
        ...
        ProductList::insert([
            ...
            'series_id' => $request -> series_id, //系列
            'image' => 'ali_oss:'.$request -> filename,
            'class_id' => $request -> class_id,
            'product_name' => $request -> product_name,
            'weight' => $request -> weight,
            ...
            foreach($res as $k => $vo){
                ...
                */
                ...
                if(isset($vo[0]) && isset($vo[1]) && isset($vo[2]) && isset($vo[3])  && isset($vo[4]) && $vo[0] && $vo[1] && $vo[2] && $vo[3] && $vo[4] ){
                    ...
                    \Illuminate\Support\Facades\Log::info($vo[2]);
                    \Illuminate\Support\Facades\Log::info($vo[3]);
                    if(!$class){
                        $fail_arr[] = $k + 1;
                        continue;
                    }
