...
class PackageController extends Controller
{
    ...
    */
                ...
                */
                ...
                if($request -> package_status){
                    ...
                    if($request -> package_status == '2'){
                        //已发货
                        $query -> whereNotNull('ems_package.id');
                    }elseif($request -> package_status == '1'){
                        //待付款
                        $query -> where('mp_temp_package_number.order_status',0) -> orWhere('order.pay_status','0') -> orWhereNull('order.pay_status');
                    }elseif($request -> package_status == '3'){
                        //待缴税
                        $query -> where('ems_package.ems_status',5);
                    }elseif($request -> package_status == '4'){
                        //异常件
                        $query -> where('ems_package.ems_status',8);
                    }
                    ...
                    if($vo -> ems_status == '8'){
                        //异常件
                        if(time() > intval($vo -> inspection_time) + 86400 * 10){
                            //需办理退运 按照异常处理
                            $list[$k] -> order_status_str = '异常件，但是不需要换地址';
                            $list[$k] -> button_num = 6;
                        }else{
                            //需办理退运 按照异常处理
                        ...
                        $list[$k] -> order_status_str = '待发货';
                        $list[$k] -> button_num = 7;
                    }elseif(!$vo -> pay_status){
                        if($vo -> status == 5){
                            //已完善 待付款
                            $list[$k] -> order_status_str = '已完善待付款';
                            $list[$k] -> button_num = 1;
                        }else{
                            //未完善 待付款
                            $list[$k] -> order_status_str = '未完善待付款';
                            $list[$k] -> button_num = 5;
                        }
                    }else{
    ...
    //更改地址
    public function changePackageAddress(Request $request){
        $validator = Validator::make($request->all(), [
            'user_id' => 'required|exists:mp_users,id',
            'package_id' => 'required',
            'address_id' => 'required',
        ...
        //先看下此人有没有这个地址
        $address = DB::table('mp_address') -> where([
            'user_id' => $request -> user_id,
            'id' => $request -> address_id
        ]) -> first();
        if(!$address){
        ...
        $package = DB::table('mp_temp_package_number') -> where([
            'id' => $request -> package_id
        ]) -> first();
        if(!$package){
            return [
        ...
        $userid = $user_model -> getWxUserId($request -> user_id);

        $order_res = $model -> underOrder([
            'weights' => [$package->weight],
            'from_area' => $package -> area_id,
            'user_id' => $userid,
            'names' => [$address->name],
            'address' => [$address->address],
    ...
    //批量付款显示页面
    public function payOrderBatch(Request $request){
        $validator = Validator::make($request->all(), [
            'user_id' => 'required|exists:mp_users,id',
        ...
        $package_ids = explode(',',$request -> package_ids);
        $price_all = 0;
        foreach($package_ids as $vo){
            ...
            $price_all += $order_info -> pay_price;
        }
    ...
    //批量付款操作
    public function payOrderBatchRes(Request $request){
